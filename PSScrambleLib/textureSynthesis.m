...
% All rights reserved.
    ...
    if (exist('imask', 'var') && ~isempty(imask))
        if  size(imask, 1) ~= numel(im)
            error('imask size %d does not match image dimensions [%d,%d]',...
...
%     figure(imf);  clf
%     subplot(1,2,1); 
%     grayRange = showIm(im,'auto',1); title('Starting image');
%     drawnow
% 
    prev_im = im;
% 
...
%     figure(imf);
%     clf;
%     showIm(im,'auto',1); title(sprintf('iteration 0'));
